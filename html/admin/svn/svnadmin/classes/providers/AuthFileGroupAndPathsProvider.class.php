												...
												\svnadmin\core\interfaces\IPathsEditProvider
{
	...
	public function getGroups()
	{
		...
		// $groupTagArray = $this->m_authfile->grouptag();
		// error_log(var_export($groupTagArray,true));
		...
		if( is_array( $groupNamesArray ) )
		{
			for( $i=0; $i<count($groupNamesArray); $i++ )
			{
				...
				$groupObj->tag = $this->m_authfile->grouptag($groupNamesArray[$i]);
	...
	{
		$retArray = array();
		$groupNamesArray = $this->m_authfile->getGroupsOfSubgroup( $objSubgroup->name );
	...
	/**
	 * (non-PHPdoc)
	 * @see svnadmin\core\interfaces.IGroupViewProvider::isSubgroupInGroup()
	 */
	public function isSubgroupInGroup($objSubgroup, $objGroup)
	{
		return $this->m_authfile->isSubgroupInGroup( $objGroup->name, $objSubgroup->name );
	}

	// 240923 태그생성
	public function updateGroupTag( $objGroup )
	{
		return $this->m_authfile->updateGroupTag($objGroup->name, $objGroup->tag);
	}

	// 250723 태그인터페이스추가
	public function updateAccessPathTag( $objGroup )
	{
		error_log("updateAccessPathTag: " . var_export($objGroup, true));
		return $this->m_authfile->updateAccessPathTag(urldecode($objGroup->path), $objGroup->tag);
	}
	...
	public function getPaths()
	{
		...
		for( $i=0; $i<count($paths); $i++ )
		{
			...
			// 250723 access-path tag 불러오기
        	$o->tag = $this->m_authfile->accesstag($paths[$i]);
