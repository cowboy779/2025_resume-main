...
class IF_SVNAuthFileC
{
	...
	private $GROUP_TAG	= 'grouptag';
	private $ACCESS_TAG	= 'accesstag'; // 250723 태그인터페이스추가
	private $ALIAS_SIGN		= '&';
	...
	public function grouptag($groupname)
	{
		$tagString = $this->config->getValue($this->GROUP_TAG, $groupname);
		
		// 가져온 값이 null이 아니면 처리합니다.
		if ($tagString != null) {
			// 각 줄을 배열로 분리합니다.
			$arrTags = explode(PHP_EOL, $tagString);

			foreach ($arrTags as $line) {
				$line = trim($line);
	...
	public function repositories()
	{
		...
		{
			if ($section != $this->GROUP_SECTION && 
			$section != $this->GROUP_TAG &&
			$section != $this->ALIAS_SECTION && 
			$section != $this->ACCESS_TAG && // 250723 태그인터페이스추가
			!empty($section)) // empty = keys without section header.
			{
				$ret[] = $section;
			}
		}
	...
	// 250723 태그구현추가
	public function accesstag($path)
	{
		$tagString = $this->config->getValue($this->ACCESS_TAG, $path);
	...
	public function updateGroupTag($groupname, $tag) {

		$this->config->removeValue($this->GROUP_TAG, $groupname);
		// 240923 새로작성 태그
		$this->config->setValue($this->GROUP_TAG, $groupname, $tag);
		return true;
	}

	public function updateAccessPathTag($path, $tag) {

		$this->config->removeValue($this->ACCESS_TAG, $path);
		// 250723 태그인터페이스추가
		$this->config->setValue($this->ACCESS_TAG, $path, $tag);
		return true;
	}
